...
void TWPartitionManager::Output_Partition(TWPartition* Part) {
	...
	printf("%s | %s | Size: %.1lfMB", Part->Mount_Point.c_str(), Part->Actual_Block_Device.c_str(), (double)Part->Size / mb);
	if (Part->Can_Be_Mounted) {
		printf(" Used: %.1lfMB Free: %.1lfMB Backup Size: %.1lfMB", (double)Part->Used / mb, (double)Part->Free / mb, (double)Part->Backup_Size / mb);
	}
	printf("\n   Flags: ");
	if (Part->Can_Be_Mounted)
